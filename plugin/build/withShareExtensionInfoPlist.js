...
const withShareExtensionInfoPlist = (config, { fonts = [], activationRules = [{ type: "text" }, { type: "url" }], backgroundColor, height, preprocessingFile, googleServicesFile, }) => {
    return (0, config_plugins_1.withInfoPlist)(config, (config) => {
        ...
        const appGroup = (0, index_1.getAppGroup)(config);
        let infoPlist = {
            ...
            NSExtension: {
                NSExtensionAttributes: {
                    NSExtensionActivationRule: (() => {
                        ...
                        const hasImage = activationRules.some((rule) => rule.type === "image");
                        const hasFile = activationRules.some((rule) => rule.type === "file");
                        const rulesToProcess = hasImage && !hasFile
                            ? [
                                ...activationRules,
                                {
                                    type: "file",
                                    max: activationRules.find((rule) => rule.type === "image")
                                        ?.max ?? 1,
                                },
                            ]
                            : activationRules;
